...
public class X_X_z_p_0_reference extends AbstractActor {
	...
	private Map<TestcasesModel.P_s, Set<TestcasesModel.X>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public X_X_z_p_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("genericWithStateTest_121_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("genericWithState_112_junction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("simpleBinding_39_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, TestcasesModel.X source, TestcasesModel.P_s target) {
	...
	private void leftAdded(NodeAddedLeft<TestcasesModel.X> message) {
		...
		TestcasesModel.P_s target = (TestcasesModel.P_s) message.input.getX_z_p();
	...
	private void addReference(ReferenceAdded<TestcasesModel.X, TestcasesModel.P_s> message) {
	...
	private void removeReference(ReferenceDeleted<TestcasesModel.X, TestcasesModel.P_s> message) {
