...
public class A_A_c_u_0_reference extends AbstractActor {
	...
	private Map<TestcasesModel.U_s, Set<TestcasesModel.A>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public A_A_c_u_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("genericWithState_107_junction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("generic_102_junction"), this::returnTrue));
		...
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("unspecifiedStateChange_130_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, TestcasesModel.A source, TestcasesModel.U_s target) {
	...
	private void leftAdded(NodeAddedLeft<TestcasesModel.A> message) {
		...
		TestcasesModel.U_s target = (TestcasesModel.U_s) message.input.getA_c_u();
	...
	private void addReference(ReferenceAdded<TestcasesModel.A, TestcasesModel.U_s> message) {
	...
	private void removeReference(ReferenceDeleted<TestcasesModel.A, TestcasesModel.U_s> message) {
