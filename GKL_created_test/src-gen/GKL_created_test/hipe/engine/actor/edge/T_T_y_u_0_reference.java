...
public class T_T_y_u_0_reference extends AbstractActor {
	...
	private Map<GKLModel.U_s, Set<EdgeMatch>> targetElements = new HashMap<GKLModel.U_s, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<GKLModel.U_s, Set<GKLModel.T>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public T_T_y_u_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("Tp_y_35_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, GKLModel.T source, GKLModel.U_s target) {
	...
	private void addPendingMatch(Object source, Object target) {
		...
		if(sourcePending == null) {
			...
			pending.put((GKLModel.U_s) target, sourcePending);
		}
	...
	private void leftAdded(NodeAddedLeft<GKLModel.T> message) {
		...
		GKLModel.U_s target = (GKLModel.U_s) message.input.getT_y_u();
	...
	private void rightAdded(NodeAddedRight<GKLModel.U_s> message) {
	...
	private void rightRemoved(NodeDeletedRight<GKLModel.U_s> message) {
	...
	private void addReference(ReferenceAdded<GKLModel.T, GKLModel.U_s> message) {
	...
	private void removeReference(ReferenceDeleted<GKLModel.T, GKLModel.U_s> message) {
