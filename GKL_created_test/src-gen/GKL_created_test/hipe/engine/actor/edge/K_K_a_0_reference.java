...
public class K_K_a_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<GKLModel.K, Set<EdgeMatch>> sourceElements = new HashMap<GKLModel.K, Set<EdgeMatch>>();
	...
	private Map<reactionContainer.Agent, Set<GKLModel.K>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public K_K_a_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("KT_x_80_nacjunction"), this::check_constraint_6));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("KT_y_82_nacjunction"), this::check_constraint_8));
		ports.add(new PortEdge(getSelf(), name2actor.get("k_aBound_production"), this::check_constraint_4));
	}	
	...
	private void addMatch(InputMessage message, GKLModel.K source, reactionContainer.Agent target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<GKLModel.K> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<GKLModel.K>();
			pending.put((reactionContainer.Agent) target, sourcePending);
		}
		sourcePending.add((GKLModel.K) source);
	}
	
	private void leftAdded(NodeAddedLeft<GKLModel.K> message) {
		...
		reactionContainer.Agent target = (reactionContainer.Agent) message.input.getK_a();
	...
	private void rightAdded(NodeAddedRight<reactionContainer.Agent> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<GKLModel.K> pendingsSources = pending.get(message.input);
			for(GKLModel.K source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<GKLModel.K> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<GKLModel.K> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<GKLModel.K, reactionContainer.Agent> message) {
	...
	private void removeReference(ReferenceDeleted<GKLModel.K, reactionContainer.Agent> message) {
		Set<GKLModel.K> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof GKLModel.K) {
			leftChanged(message);
		}
		else {
	...
	public boolean check_constraint_6(EdgeMatch edge) {
		GKLModel.K src = (GKLModel.K) edge.source();
