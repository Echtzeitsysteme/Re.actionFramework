...
public class T_T_y_0_reference extends AbstractActor {
	...
	private Map<reactionContainer.Agent, Set<GKLModel.T>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public T_T_y_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("KT_y_81_nacjunction"), this::check_constraint_7));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("PT_y_86_nacjunction"), this::check_constraint_12));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("T_pp_unbound_88_nacjunction"), this::check_constraint_14));
		ports.add(new PortEdge(getSelf(), name2actor.get("t_yBound_production"), this::check_constraint_3));
	}	
	...
	private void addMatch(InputMessage message, GKLModel.T source, reactionContainer.Agent target) {
	...
	private void leftAdded(NodeAddedLeft<GKLModel.T> message) {
		...
		reactionContainer.Agent target = (reactionContainer.Agent) message.input.getT_y();
	...
	private void addReference(ReferenceAdded<GKLModel.T, reactionContainer.Agent> message) {
	...
	private void removeReference(ReferenceDeleted<GKLModel.T, reactionContainer.Agent> message) {
	...
	public boolean check_constraint_7(EdgeMatch edge) {
		GKLModel.T src = (GKLModel.T) edge.source();
