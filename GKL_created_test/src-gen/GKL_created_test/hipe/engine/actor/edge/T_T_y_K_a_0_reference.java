package GKL_created_test.hipe.engine.actor.edge;
...
public class T_T_y_K_a_0_reference extends AbstractActor {
	...
	private Map<GKLModel.K, Set<EdgeMatch>> targetElements = new HashMap<GKLModel.K, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<GKLModel.K, Set<GKLModel.T>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public T_T_y_K_a_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("KT_x_94_nacjunction"), this::returnTrue));
		ports.add(new PortEdge(getSelf(), name2actor.get("KT_yBwd_production"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("KT_y_95_nacjunction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("KT_y_98_nacjunction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("PT_y_105_nacjunction"), this::returnTrue));
		ports.add(new PortEdge(getSelf(), name2actor.get("T_y_K_aBoundSrc_production"), this::returnTrue));
		ports.add(new PortEdge(getSelf(), name2actor.get("T_y_K_aBoundTrg_production"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("Tp_y_47_junction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("obs_T_pp_unbound_109_nacjunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, GKLModel.T source, GKLModel.K target) {
	...
	private void addPendingMatch(Object source, Object target) {
		...
		if(sourcePending == null) {
			...
			pending.put((GKLModel.K) target, sourcePending);
		}
	...
	private void leftAdded(NodeAddedLeft<GKLModel.T> message) {
		...
		GKLModel.K target = (GKLModel.K) message.input.getT_y_K_a();
	...
	private void rightAdded(NodeAddedRight<GKLModel.K> message) {
	...
	private void rightRemoved(NodeDeletedRight<GKLModel.K> message) {
	...
	private void addReference(ReferenceAdded<GKLModel.T, GKLModel.K> message) {
	...
	private void removeReference(ReferenceDeleted<GKLModel.T, GKLModel.K> message) {
