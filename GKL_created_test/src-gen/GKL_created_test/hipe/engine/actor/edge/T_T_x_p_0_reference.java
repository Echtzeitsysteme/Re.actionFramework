...
public class T_T_x_p_0_reference extends AbstractActor {
	...
	private Map<GKLModel.P_s, Set<EdgeMatch>> targetElements = new HashMap<GKLModel.P_s, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<GKLModel.P_s, Set<GKLModel.T>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public T_T_x_p_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("Tu_x_61_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("obs_T_pp_81_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("obs_T_pp_unbound_86_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, GKLModel.T source, GKLModel.P_s target) {
	...
	private void addPendingMatch(Object source, Object target) {
		...
		if(sourcePending == null) {
			...
			pending.put((GKLModel.P_s) target, sourcePending);
		}
	...
	private void leftAdded(NodeAddedLeft<GKLModel.T> message) {
		...
		GKLModel.P_s target = (GKLModel.P_s) message.input.getT_x_p();
	...
	private void rightAdded(NodeAddedRight<GKLModel.P_s> message) {
	...
	private void rightRemoved(NodeDeletedRight<GKLModel.P_s> message) {
	...
	private void addReference(ReferenceAdded<GKLModel.T, GKLModel.P_s> message) {
	...
	private void removeReference(ReferenceDeleted<GKLModel.T, GKLModel.P_s> message) {
