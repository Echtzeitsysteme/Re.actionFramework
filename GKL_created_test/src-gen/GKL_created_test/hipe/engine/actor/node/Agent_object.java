...
public class Agent_object extends AbstractActor {
	
	private List<Port<reactionContainer.Agent>> ports;		
	...
	public void initActor(InitActor m) {
		...
		ports.add(new PortNodeRight<reactionContainer.Agent>(getSelf(), name2actor.get("K_K_a_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<reactionContainer.Agent>(getSelf(), name2actor.get("P_P_a_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<reactionContainer.Agent>(getSelf(), name2actor.get("T_T_x_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<reactionContainer.Agent>(getSelf(), name2actor.get("T_T_y_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<reactionContainer.Agent> message) {
		for(Port<reactionContainer.Agent> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<reactionContainer.Agent> message) {
		for(Port<reactionContainer.Agent> port : ports) {
	...
	private void changeAttribute(AttributeChanged<reactionContainer.Agent> message) {
		for(Port<?> port : ports) {
