...
public class U_s_object extends AbstractActor {
	
	private List<Port<GKLModel.U_s>> ports;		
	...
	public void initActor(InitActor m) {
		...
		ports.add(new PortNodeRight<GKLModel.U_s>(getSelf(), name2actor.get("T_T_x_u_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<GKLModel.U_s>(getSelf(), name2actor.get("T_T_y_u_0_reference"), this::returnTrue));
		ports.add(new PortNodeMatchLeft<GKLModel.U_s>(getSelf(), name2actor.get("Tu_x_59_junction"), this::returnTrue));
		ports.add(new PortNodeMatchLeft<GKLModel.U_s>(getSelf(), name2actor.get("Tu_y_73_junction"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<GKLModel.U_s> message) {
		for(Port<GKLModel.U_s> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<GKLModel.U_s> message) {
		for(Port<GKLModel.U_s> port : ports) {
	...
	private void changeAttribute(AttributeChanged<GKLModel.U_s> message) {
		for(Port<?> port : ports) {
