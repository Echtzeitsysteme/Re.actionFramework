...
public class P_s_object extends AbstractActor {
	
	private List<Port<GKLModel.P_s>> ports;		
	...
	public void initActor(InitActor m) {
		...
		ports.add(new PortNodeRight<GKLModel.P_s>(getSelf(), name2actor.get("T_T_x_p_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<GKLModel.P_s>(getSelf(), name2actor.get("T_T_y_p_0_reference"), this::returnTrue));
		ports.add(new PortNodeMatchLeft<GKLModel.P_s>(getSelf(), name2actor.get("Tp_x_31_junction"), this::returnTrue));
		ports.add(new PortNodeMatchLeft<GKLModel.P_s>(getSelf(), name2actor.get("Tp_y_45_junction"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<GKLModel.P_s> message) {
		for(Port<GKLModel.P_s> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<GKLModel.P_s> message) {
		for(Port<GKLModel.P_s> port : ports) {
	...
	private void changeAttribute(AttributeChanged<GKLModel.P_s> message) {
		for(Port<?> port : ports) {
